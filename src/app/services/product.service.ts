...
import { CategoryDTO, CategoryForEditDTO, CategoryTreeItemDTO } from '../DTOs/category/CategoryDTO';
...
import { ProductGalleryDTO, ProductGalleryMiniDTO } from '../DTOs/product/ProductGalleryDTO';
...
export class ProductService {
  ...
  addProduct(prd: ProductDTO): Observable<ProductDTO> {
    return this.http.post<IResponseResult<ProductDTO>>('/adminproduct/add-product', prd).pipe(
      map(res => {
  ...
  getProductForEdit(id: number): Observable<ProductDTO> {
    if (!id)
      return of(null);
      
    return this.http.get<IResponseResult<ProductDTO>>('/adminproduct/get-product-for-edit/' + id).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data;
        // else if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<ProductDTO>(`دریافت اطلاعات محصول :${id}`))
    );
  }

  editProduct(product: ProductDTO): Observable<ProductDTO> {
    return this.http.post<IResponseResult<ProductDTO>>('/adminproduct/edit-product', product).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data;
        else// if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<ProductDTO>(`ویرایش اطلاعات محصول :${product.id}`))
    );
  }
  ...
  getFilteredProducts(filter: productDatasourceDTO): Observable<productDatasourceDTO> {
    ...
    return this.http.get<IResponseResult<productDatasourceDTO>>('/adminproduct/filter-products', { params }).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data;
        else
  ...
  addCategory(ctg: CategoryDTO): Observable<boolean> {
    return this.http.post<IResponseResult<any>>("/adminproduct/add-category", ctg).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return true;
        else// if (res.eStatus === Status.NotFound)
  ...
  getCategoryForEdit(id: number): Observable<CategoryForEditDTO> {
    return this.http.get<IResponseResult<CategoryForEditDTO>>('/adminproduct/get-category-for-edit/' + id).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data
        else
      ...
      catchError(this.errorHandler.handleError<CategoryForEditDTO>(`دریافت اطلاعات کتگوری :${id}`))
    );
  }

  editCategory(ctg: CategoryDTO): Observable<boolean> {
    return this.http.post<IResponseResult<any>>('/adminproduct/edit-category', ctg).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return true;
        else// if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<boolean>(`ویرایش اطلاعات کتگوری : ${ctg.id}`))
    );
  }

  public GetCategoryChildren(id: number): Observable<CategoryDTO[]> {
    console.log("getting sub categories from server", id);
    return this.http.get<IResponseResult<CategoryDTO[]>>('/products/child-categories/' + id).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data;
        else// if (res.eStatus === Status.NotFound)
  ...
  public GetCategoryTree(): Observable<CategoryTreeItemDTO[]> {
    return this.http.get<IResponseResult<CategoryTreeItemDTO[]>>('/adminproduct/get-tree').pipe(
      map(res=>{
        if (res.eStatus === Status.Success)
          return res.data;
        else// if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<CategoryTreeItemDTO[]>(`دریافت کامل کتگوری`))
    );
  }
  //#endregion
  //---------------------------------------
  ...
  getProductGalleries(productId: number): Observable<ProductGalleryMiniDTO[]> {
    return this.http.get<IResponseResult<ProductGalleryMiniDTO[]>>('/adminproduct/product-galleries/' + productId).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return res.data;
        else
  ...
  addGallery(gal: ProductGalleryDTO): Observable<boolean> {
    return this.http.post<IResponseResult<any>>('/adminproduct/add-gallery', gal).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return true;
        else// if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<ProductDTO>(`ثبت گالری جدید برای محصول ${gal.productId}`))
    );
  }

  deleteGallery(id: number): Observable<boolean> {
    return this.http.get<IResponseResult<any>>('/adminproduct/delete-gallery/'+ id).pipe(
      map(res => {
        if (res.eStatus === Status.Success)
          return true;
        else// if (res.eStatus === Status.NotFound)
      ...
      catchError(this.errorHandler.handleError<ProductDTO>(`حذف گالری ${id} برای محصول `))
    );
  }
  //#endregion
}
